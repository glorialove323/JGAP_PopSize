...
public class Population implements Serializable, ICloneable, IPersistentRepresentation {
    ...
    /*
     * ====================================================================
     * 用于计算lifetime 包括getBestFit(),getWorstFit(),getAvgFit(),getLifeTime()
     * mLifetime,MINLIFETIME,MAXLIFETIME
     * ===================================================================
     */
     ...
     * @since 3.0
     */
    public Population(final Configuration a_config) throws InvalidConfigurationException {
        this(a_config, 100);
    }

    /*
     * Constructs the Population from a list of Chromosomes. Does not use
     * cloning!
     * 
    ...
    public Population(final Configuration a_config, final IChromosome[] a_chromosomes)
        ...
        synchronized (m_chromosomes) {
            for (int i = 0; i < a_chromosomes.length; i++) {
                // In here we could test for null chromosomes, but this is
                // skipped
                // because of performance issues (although this may seem
                // idiotic...)
     ...
     * @since 3.2
     */
    public Population(final Configuration a_config, final IChromosome a_chromosome)
            throws InvalidConfigurationException {
        this(a_config, 1);
     ...
     * @since 2.0
     */
    public Population(final Configuration a_config, final int a_size) throws InvalidConfigurationException {
        if (a_config == null) {
     ...
     * 
     * @param a_chromosomes
     *            the chromosomes to make the population up from
     * 
     * @author Klaus Meffert
     */
    ...
    /**
     * Sets in the given Chromosome on the given index in the list of
     * chromosomes. If the given index is exceeding the list by one, the
     * chromosome is appended.
     * 
     * @param a_index
     *            the index to set the Chromosome in
     * @param a_chromosome
     *            the Chromosome to be set
     * 
     * @author Klaus Meffert
     * @since 2.0
     */
     ...
     *         retrieved list by using clear(), remove(int) etc. If you do so,
     *         you need to call setChanged(true)
     * 
     * @author Klaus Meffert
     * @since 2.0
     */
    ...
    /**
     * @param a_index
     *            the index of the Chromosome to be returned
    ...
    /**
     * @return Iterator for the Chromosome list in the Population. Please be
     *         aware that using remove() forces you to call setChanged(true)
     * 
     * @author Klaus Meffert
     * @since 2.0
     */
    ...
    public IChromosome[] toChromosomes() {
        return (IChromosome[]) m_chromosomes.toArray(new IChromosome[m_chromosomes.size()]);
    }
     ...
     * 
     * @param a_startIndex
     *            index to begin the evaluation with
     * @param a_endIndex
     *            index to end the evaluation with
     ...
     * 
     * @param a_changed
     *            true: population's fittest chromosome may have changed, false:
     *            fittest chromosome evaluated earlier is still valid
     * 
     * @author Klaus Meffert
     * @since 2.2
     */
    ...
    /**
     * @return true: population's chromosomes (maybe) were changed, false: not
     *         changed for sure
     * 
     * @since 2.6
     */
     ...
     * 
     * @param a_sorted
     *            true: mark population as sorted
     * 
     * @author Klaus Meffert
     * @since 2.6
     */
    ...
    /**
     * Determines whether the given chromosome is contained within the
     * population.
     * 
     * @param a_chromosome
     ...
     * 
     * @param a_index
     *            index of chromosome to be removed in list
    ...
    public void keepPopSizeConstant() throws InvalidConfigurationException {
        ...
        // getConfiguration().getKeepPopConstantSelector();
        // if (popSize > maxSize) {
        ...
        while (popSize > maxSize) {
            if (monitorActive) {
                // Fire monitor with population and index of chromosome to be
                // removed.
                // -------------------------------------------------------------------
                monitor.event(IEvolutionMonitor.MONITOR_EVENT_REMOVE_CHROMOSOME, getConfiguration().getGenerationNr(),
    ...
    /**
     * Sorts the Chromosome list and returns the fittest n Chromosomes in the
     * population.
     * 
     * @param a_numberOfChromosomes
     *            number of top performer chromosomes to be returned
     * @return list of the fittest n Chromosomes of the population, or the
     *         fittest x Chromosomes with x = number of chromosomes in case n >
     *         x.
     * 
    ...
    public List determineFittestChromosomes(final int a_numberOfChromosomes) {
        int numberOfChromosomes = Math.min(a_numberOfChromosomes, getChromosomes().size());
     ...
     * value using ChromosomFitnessComparator. The fittest chromosome is then at
     * index 0.
     * 
     * @author Klaus Meffert
     * @since 2.6
    ...
    /**
     * Sorts the chromosomes within the population utilzing the given
     * comparator.
     * 
     * @param a_comparator
     *            the comparator to utilize for sorting
     * 
     * @author Klaus Meffert
     * @since 2.6
     */
     ...
     * Population.
     * 
     * @param a_resolveCompositeGenes
     *            true: split encountered CompositeGenes into their single
     *            (atomic) genes
    ...
    /**
     * This method is not producing symmetric results as -1 is more often
     * returned than 1 (see description of return value).
     * 
     * @param a_pop
     *            the other population to compare
     * @return 1: a_pop is null or having fewer chromosomes or equal number of
     *         chromosomes but at least one not contained. 0: both populations
     *         containing exactly the same chromosomes. -1: this population
     *         contains fewer chromosomes than a_pop
     * 
     * @author Klaus Meffert
     * @since 2.6
     */
    ...
    /**
     * Returns a persistent representation of this chromosome, see interface
     * Gene for description. Similar to CompositeGene's routine. But does not
     * include all information of the chromosome (yet).
     * 
    ...
    public String getPersistentRepresentation() {
        ...
        for (int i = 0; i < m_chromosomes.size(); i++) {
            ...
            if (!(chrom instanceof IPersistentRepresentation)) {
                throw new RuntimeException("Population contains a chromosome of type " + chrom.getClass().getName()
                        + " which does not implement" + " IPersistentRepresentation!");
            }
            b.append(CHROM_DELIMITER_HEADING);
            b.append(StringKit.encode(chrom.getClass().getName() + CHROM_DELIMITER
                    + ((IPersistentRepresentation) chrom).getPersistentRepresentation()));
     ...
     * 
     * @param a_chromClassName
     *            name of the Chromosome class
     * @param a_persistentRepresentation
     *            persistent representation of the Chromosome to create (could
     *            be obtained via getPersistentRepresentation)
     * 
    ...
    protected IChromosome createChromosome(String a_chromClassName, String a_persistentRepresentation) throws Exception {
        ...
        IChromosome chrom = (IChromosome) constr.newInstance(new Object[] { getConfiguration() });
        ((IPersistentRepresentation) chrom).setValueFromPersistentRepresentation(a_persistentRepresentation);
        return chrom;
    }
    ...
    protected static final List split(String a_string) throws UnsupportedRepresentationException {
        ...
        // Chromosome data.
        // ----------------
        StringTokenizer st = new StringTokenizer(a_string, CHROM_DELIMITER_HEADING + CHROM_DELIMITER_CLOSING, true);
    ...
    public double getWorstFit() {
        ...
        while (it.hasNext()) {
            IChromosome chrom = (IChromosome) it.next();
            fitness = chrom.getFitnessValue();
            if (evaluator.isFitter(worstFitness, fitness)) {
